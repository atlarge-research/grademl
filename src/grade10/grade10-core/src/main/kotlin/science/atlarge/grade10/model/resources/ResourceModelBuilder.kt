...
import science.atlarge.grade10.util.DEFAULT_TIMESLICE_LENGTH
...
interface ResourceBuilder {
    ...
    fun newBlockingMetric(
            name: String,
            blockedTimeSlices: TimeSlicePeriodList
...
interface BlockingMetricBuilder {
    ...
    val blockedTimeSlices: TimeSlicePeriodList
...
class ResourceBuilderImpl(
    ...
    override fun newBlockingMetric(name: String, blockedTimeSlices: TimeSlicePeriodList): BlockingMetricBuilder {
        val type = lookupBlockingMetricType(name)
        require(type !in blockingMetricBuilders) { "Metric with type \"${type.path}\" already exists" }
        return BlockingMetricBuilderImpl(type, blockedTimeSlices)
...
class BlockingMetricBuilderImpl(
        ...
        override val blockedTimeSlices: TimeSlicePeriodList
) : BlockingMetricBuilder {

    fun build(parent: Resource): Metric.Blocking {
        return Metric.Blocking(type, parent, blockedTimeSlices)
    }
...
fun buildResourceModel(
    ...
    val rootResource =
            ResourceBuilderImpl(resourceModelSpecification.rootResourceType, "")
                    .also(init)
                    .build()
    return ResourceModel(resourceModelSpecification, rootResource, T)
}
...
fun mergeResourceModels(resourceModels: List<ResourceModel>): ResourceModel {
    fun ResourceBuilder.copy(resources: List<Resource>) {
        ...
        resources.flatMap { it.metrics.values }
                .forEach { metric ->
                    when (metric) {
                        is Metric.Blocking -> newBlockingMetric(metric.name, metric.blockedTimeSlices)
                        is Metric.Consumable -> newConsumableMetric(metric.name, metric.observedUsage, metric.capacity)
                    }
                }
    }

    val T = resourceModels.firstOrNull()?.T ?: Time(DEFAULT_TIMESLICE_LENGTH)
    require(resourceModels.all { it.T == T }) {
        "Resource models must use the same timeslice granularity to merge them"
    }

    val mergedModelSpecification = mergeResourceModelSpecifications(resourceModels.map { it.specification })
    return buildResourceModel(mergedModelSpecification, T) {
