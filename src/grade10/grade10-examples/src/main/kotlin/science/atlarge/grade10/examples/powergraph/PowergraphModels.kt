...
import science.atlarge.grade10.metrics.TimeSlicePeriodList.Companion.toSlices
...
object PowergraphResourceModel {
    ...
    fun fromLogDirectories(logDirectories: List<Path>, T: Time): ResourceModel {
                    ...
                    newSubresource(WORKER_NAME, instanceId = workerId) {
                        threads.forEach { threadId, metricData ->
                            newSubresource(THREAD_NAME, instanceId = threadId) {
                                newBlockingMetric(READ_GATHER_MSG_NAME, metricData.toSlices(T))
                            }
                        }
                    }
                }
