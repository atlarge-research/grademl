...
class PowergraphJob(
    ...
    override fun parseInput(T: Time): Pair<ExecutionModel, ResourceModel> {
        val (executionModel, powergraphResourceModel) = parsePowergraphLogs(T)
    ...
    private fun parsePowergraphLogs(T: Time): Pair<ExecutionModel, ResourceModel> {
        ...
        val executionModel = jobExecutionModelFromRecords(records, PowergraphExecutionModel.specification, T)

        // Build the Powergraph resource model
        val resourceModel = PowergraphResourceModel.fromLogDirectories(inputDirectories, T)
