...
class GiraphJob(
    ...
    override fun parseInput(T: Time): Pair<ExecutionModel, ResourceModel> {
        val (executionModel, giraphResourceModel) = parseGiraphLogs(T)
    ...
    private fun parseGiraphLogs(T: Time): Pair<ExecutionModel, ResourceModel> {
        ...
        val executionModel = jobExecutionModelFromRecords(records, GiraphExecutionModel.specification, T)

        // Build the Giraph resource model
        val resourceModel = GiraphResourceModel.fromRecords(records, T)
...
private object GiraphResourceAttributionRuleProvider : ResourceAttributionRuleProvider() {
    ...
    private val LOAD_GRAPH_PATH = Path.absolute("LoadGraph")
    private val CLEANUP_PATH = Path.absolute("Cleanup")
    ...
    private val NET_BYTES_TRANSMITTED_PATH = Path.absolute("perf-monitor", "machine", "network",
            "bytes-transmitted")
    private val DISK_UTILIZATION_PATH = Path.absolute("perf-monitor", "machine", "disk", "total-utilization")
    ...
    override fun getRuleForConsumableResource(
        ...
        return when (metricType.path) {
            CPU_UTILIZATION_PATH -> {
                if (phaseType.path in COMPUTE_THREAD_PATH) {
                    ConsumableResourceAttributionRule.Exact(1.0)
                } else {
                    ConsumableResourceAttributionRule.Variable(1.0)
                }
            }
            in listOf(NET_BYTES_RECEIVED_PATH, NET_BYTES_TRANSMITTED_PATH) -> {
                when (phaseType.path) {
                    COMMUNICATE_PATH -> ConsumableResourceAttributionRule.Variable(1.0)
                    in WORKER_SUPERSTEP_PATH -> ConsumableResourceAttributionRule.None
                    else -> ConsumableResourceAttributionRule.Variable(1.0)
                }
            }
            DISK_UTILIZATION_PATH -> {
                if (phaseType.path in LOAD_GRAPH_PATH || phaseType.path in CLEANUP_PATH) {
                    ConsumableResourceAttributionRule.Variable(1.0)
                } else {
