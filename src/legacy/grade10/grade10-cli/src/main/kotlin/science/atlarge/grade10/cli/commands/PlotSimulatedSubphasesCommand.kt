...
import science.atlarge.grade10.cli.data.SimulatedPhaseListWriter
...
object PlotSimulatedSubphasesCommand : Command {

    private const val SCRIPT_FILENAME = "plot-simulated-subphases.R"

    override val name: String
        get() = "plot-simulated-subphases"
    ...
    override fun process(arguments: List<String>, cliState: CliState) {
        ...
        if (filteredPhases.isEmpty()) {
            println()
            println("Plotting simulated subphases for 0 phases")
            println("  (all matching phases were excluded using --exclude-type)")
            return
        }

        println()
        println("Plotting simulated subphases for ${phases.size} phase${if (phases.size > 1) "s" else ""}:")
        for (phase in phases) {
    ...
    private fun exportForPhase(
        ...
        writeSimulatedPhaseListFile(dataOutputPath, phase, cliState, force)
    ...
    private fun writeSimulatedPhaseListFile(dataOutputPath: Path, phase: Phase, cliState: CliState, force: Boolean) {
        val phases = collectTreeNodes(phase) { it.subphases.values }.toList()
        val phaseListFile = dataOutputPath.resolve(SimulatedPhaseListWriter.FILENAME).toFile()
        if (!phaseListFile.exists() || force) {
            SimulatedPhaseListWriter.output(
                    phaseListFile,
                    cliState.grade10JobResult.performanceIssueIdentificationResult.simulatedPhaseExecutionResult,
                    phase,
                    phases,
                    cliState.phaseList
            )
