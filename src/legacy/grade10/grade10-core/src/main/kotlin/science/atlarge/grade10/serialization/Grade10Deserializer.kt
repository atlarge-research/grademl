...
class Grade10Deserializer private constructor(inputStream: InputStream) : Input(inputStream) {
    ...
    private fun readExecutionModel(executionModel: ExecutionModel) {
        ...
        // 0. Check that the timeslice granularity of the execution model and the serialized data match
        val storedT = readVarLong(true)
        require(storedT == executionModel.T.nanosecondsPerTimeslice) {
            "Mismatch in timeslice granularity: expected ${executionModel.T.nanosecondsPerTimeslice}, got $storedT"
        }
        // 1. Number of phases
    ...
    private fun readResourceModel(resourceModel: ResourceModel) {
        ...
        // 0. Check that the timeslice granularity of the resource model and the serialized data match
        val storedT = readVarLong(true)
        require(storedT == resourceModel.T.nanosecondsPerTimeslice) {
            "Mismatch in timeslice granularity: expected ${resourceModel.T.nanosecondsPerTimeslice}, got $storedT"
        }
