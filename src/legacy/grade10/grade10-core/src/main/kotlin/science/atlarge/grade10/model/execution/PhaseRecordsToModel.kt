...
fun jobExecutionModelFromRecords(
    ...
    val rootPhase = Phase(executionModelSpecification.rootPhaseType, null,
            T.timeSliceForStartTimestamp(rootPhaseRecords[0].startTime),
            T.timeSliceForEndTimestamp(rootPhaseRecords[0].endTime))
    ...
    depthFirstSearch(executionModelSpecification.rootPhaseType, { it.subphaseTypes.values }) { type, _ ->
        if (!type.isRoot) {
            ...
            recordsForType.forEach { record ->
                ...
                createdPhases[record.phasePath] = Phase(record.phaseType, parentPhase,
                        T.timeSliceForStartTimestamp(record.startTime),
                        T.timeSliceForEndTimestamp(record.endTime),
                        repetitionId)
            }
        }
    }

    return ExecutionModel(executionModelSpecification, rootPhase, T)
}
