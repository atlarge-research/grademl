...
data class PhaseRecord(
    ...
    override fun toString(): String =
            "PhaseRecord(phasePath=$phasePath, timestamps=[$startTime, $endTime])"
...
class PhaseRecordDerivationRule(
    ...
    private fun getPhaseTypeAndPathForMatched(record: EventRecord): Pair<PhaseType, Path> {
        val phaseType = getPhaseTypeForName(record.tags.getValue(TAG_PHASE), record.tags)
        return Pair(
                phaseType,
                Phase.constructPhasePath(phaseType) { repetitionKey -> record.tags.getValue(repetitionKey) }
        )
    }
