...
typealias BottleneckPredicateProvider = () -> PerPhaseBottleneckPredicate
...
class GroupByPhaseTypeBottleneckPredicateOPT(
        private val sameTypePredicate: PerPhaseBottleneckPredicate,
        private val differentTypePredicate: PerPhaseTypeBottleneckPredicate
) : PerPhaseBottleneckPredicate {

    private var subphases = Array<Phase?>(8) { null }
    private var subphaseStatus = BottleneckStatusArray(8)
    private var subphaseCount = 0

    private var combinedTypes = Array<PhaseType?>(8) { null }
    private var combinedStatus = BottleneckStatusArray(8)
    private var combinedCount = 0

    private val subphaseStatusIterator = object : Iterator<PhaseAndBottleneckStatus> {

        private lateinit var phaseType: PhaseType
        private var index = -1
        private val cache = PhaseAndBottleneckStatus()

        fun reset(phaseType: PhaseType) {
        ...
        private fun findNext() {
            index++
            while (index < subphaseCount && subphases[index]!!.type !== phaseType) {
                index++
            }
        }

        override fun hasNext() = index < subphaseCount

        override fun next(): PhaseAndBottleneckStatus {
            val i = index
            findNext()
            cache.phase = subphases[i]!!
            cache.bottleneckStatus = subphaseStatus[i]
            return cache
        }

    }

    private val subphaseTypeStatusIterator = object : Iterator<PhaseTypeAndBottleneckStatus> {
        ...
        override fun hasNext() = index < combinedCount

        override fun next(): PhaseTypeAndBottleneckStatus {
            cache.bottleneckStatus = combinedStatus[index]
            cache.phaseType = combinedTypes[index]!!
            index++
            return cache
        }
    ...
    override fun combineSubPhaseBottlenecks(subPhaseBottlenecks: Iterator<PhaseAndBottleneckStatus>): BottleneckStatus {
        ...
        combinePerSubphaseType()
        subphaseTypeStatusIterator.reset()
        return differentTypePredicate.combineSubPhaseTypeBottlenecks(subphaseTypeStatusIterator)
    }

    private fun add(phaseAndBottleneckStatus: PhaseAndBottleneckStatus) {
        if (subphases.size == subphaseCount) {
            subphases = subphases.copyOf(subphases.size + subphases.size / 2)
            subphaseStatus = subphaseStatus.copyOf(subphaseStatus.size + subphaseStatus.size / 2)
        }
        subphases[subphaseCount] = phaseAndBottleneckStatus.phase
        subphaseStatus[subphaseCount] = phaseAndBottleneckStatus.bottleneckStatus
        subphaseCount++
    }

    private fun combinePerSubphaseType() {
        combinedCount = 0
        for (i in 0 until subphaseCount) {
            val nextType = subphases[i]!!.type
            var typeAlreadyProcessed = false
            for (j in 0 until combinedCount) {
                if (nextType === combinedTypes[j]) {
                    typeAlreadyProcessed = true
                    break
                }
            }
            if (!typeAlreadyProcessed) {
                subphaseStatusIterator.reset(nextType)
                combinedTypes[combinedCount] = nextType
                combinedStatus[combinedCount] = sameTypePredicate.combineSubPhaseBottlenecks(subphaseStatusIterator)
                combinedCount++
            }
        }
    }
