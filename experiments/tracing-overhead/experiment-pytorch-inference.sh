...
EXP_LOG_DIR="$ROOT_DIR/logs/$(date +%Y%m%d-%H%M)_pytorch-inference"
mkdir -p "$EXP_LOG_DIR"
...
for run in $(seq 1 $REPEATS); do
	for profile_type in none trace_per_batch trace; do
		echo "======================================================================"
		echo "STARTING PYTORCH INFERENCE EXPERIMENT WITH PROFILE $profile_type ($run/$REPEATS)"
		echo "======================================================================"
		echo

		RUN_LOG_DIR="$EXP_LOG_DIR/profile_$profile_type/$run"
		mkdir -p "$RUN_LOG_DIR"
		python src/pytorch_inference_resnet50.py \
